...
t_new := Char0Wrapper(t);
which := 4;
F := FunctionFields(t_new)[which];
phi := BelyiMaps(t_new)[which];
auts := FunctionFieldAutomorphisms(t_new)[which];
K := ConstantField(F);
...
if #candidates_galois gt 0 then
  ...
  candidates, auts_ext := GetCandidateFunctionsOverExtensionChar0(Kextcyc, phi, ram, auts);
